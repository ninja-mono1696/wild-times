...
const Games = () => {
    ...
    function isOverlap(bomb:any,b:any){
        if (bomb.x < b.x + b.w && bomb.x + bomb.w > b.x && bomb.y < b.y + b.h && bomb.y + bomb.h > b.y) {
        return true
        }
        ...
        if (ref.current) {
            ...
            for(let i = 0; i<enemy.length;i++){
                if(isOverlap(enemy[i],player)){
                    ...
                    for(let i = 0; i < enemy.length; i++) {
                     canvas?.drawImage(Bomb,enemy[i].x,enemy[i].y,enemy[i].w,enemy[i].h)
                     }
                     //drawing the bird
  ...
  return (
    <div className={styles.games}>
        ...
        ></video>
        <Box display={"flex"} flexDirection = "column" gap={"10px"} position={"absolute"} top={"5%"} width={"100%"} >
        <Box  width={"100%"} 
          display={"flex"}
